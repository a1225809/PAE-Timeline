...
var LoadImage = function(position,year) {	
	...
	$('body').append("<div onclick='GetCoinsForYear(this)' id='"+year+"-img' data-year='"+year+"' style='padding:2px;z-index:99;position:absolute;top:"+viewportHeight*0.8+"px;left:"+position+"px' class='coin'><img src='"+coin.url+"' width='"+imageWidth+"' ></div>");
...
var DrawCoinWindow = function() {
	...
	$("#coin_window").append("<h2>Coins beginning with year "+currentCoins[0].from+"</h2><span style='position:absolute;top:0;right:0;z-index:100000' onclick='CloseCoinWindow()'>Close</span>");
...
var LoadCoinsForWindow = function() {
	for(var i = currentCoinsStartIndex; i < currentCoinsEndIndex; i++) {
		$("#coin_container").append("<div class='coin_list'><div class='coin_list_img'><img width='50' src='"+currentCoins[i].image_urls[0]+"'></div><div class='coin_list_title'>"+currentCoins[i].title+"</div></div><div style='clear:both'></div><hr>");
	}
